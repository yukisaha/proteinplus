...
public interface OrderRepository extends JpaRepository<Order, Long> {
    ...
    List<Order> findByUserIdCancelList(@Param("userId") Long userId, Pageable pageable);

    // 주문 상태가 CANCEL이 아닌 주문 목록 조회 (회원)
    @Query("SELECT o FROM Order o WHERE o.user.id = :userId AND o.orderStatus != 'CANCEL' ORDER BY o.orderDate desc")
    List<Order> findByUserIdOrderList(@Param("userId") Long userId, Pageable pageable);
